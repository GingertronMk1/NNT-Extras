...
fellowAdj' ((a, i):as) d done = let newFellows = [n | n <- allFellows (head a) d, not (elem n done || elem n a)] in [(new:a, i+1) | new <- newFellows] ++ fellowAdj' as d (newFellows ++ done)
...
fellowAdj as d done = let newList = fellowAdj' as d done in newList ++ fellowAdj newList d (map (head . fst) newList ++ done)

allAdj :: Actor -> [Details] -> [Adj]   -- And allAdj takes fellowAdj and wraps it all up neatly
allAdj a d = let baa = baseAdj a in baa ++ fellowAdj baa d []  -- so all you've to do is supply an Actor and a Detail list

adjLim :: Int -> Actor -> [Details] -> [Adj]  --allAdj is in theory an infinite list, so we use adjLim to limit it
adjLim l a d = takeWhile ((<=l) . snd) (allAdj a d)

adjSearch :: Actor -> Actor -> [Details] -> Adj -- adjSearch now takes the list generated by adjLim and if no list starts with the searched Actor, returns the two Actors and 1000 as an error code
adjSearch a1 a2 d = let alList = filter ((== a1) . head . fst) (adjLim limit a2 d) in if alList == [] then ([a1,a2], 1000) else head alList
